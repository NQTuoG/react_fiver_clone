...
const UserHeader = () => {
  ...
  const [isFormSearchVisible, setIsFormSearchVisible] = useState(false); // New state
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Show CategoryMenu and FormSeachProduct when scrolled down any amount
      if (scrollPosition > 0) {
        setIsCategoryMenuVisible(true);
        setIsHeaderPinned(true);
        setIsFormSearchVisible(true); // Show FormSeachProduct on scroll
      } else {
        setIsCategoryMenuVisible(false);
        setIsHeaderPinned(false);
        setIsFormSearchVisible(false); // Hide FormSeachProduct when at top
      }
    };
  ...
  return (
    <>
      ...
      >
        ...
        {isCategoryMenuVisible && <CategoryMenu />}
      </header>
    </>
  );
};
